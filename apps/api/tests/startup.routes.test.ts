...
describe('startup routes', () => {
  test('reject unauthenticated startup access and require an active workspace for authenticated founders', async () => {
    ...
    expect(unauthenticatedCreatePayload.error).toMatchObject({ code: 'AUTH_REQUIRED' });
    ...
    expect(noWorkspaceListPayload.error).toMatchObject({ code: 'ACTIVE_WORKSPACE_REQUIRED' });
  });

  test('validate malformed startup inputs before persistence', async () => {
    const { cookie } = await createAuthenticatedSession('validation@example.com');
    ...
    expect(blankNamePayload.error).toMatchObject({ code: 'STARTUP_NAME_REQUIRED', field: 'name' });
    ...
    expect(invalidStagePayload.error).toMatchObject({ code: 'STARTUP_STAGE_INVALID', field: 'stage' });
    ...
    expect(invalidTimezonePayload.error).toMatchObject({ code: 'STARTUP_TIMEZONE_INVALID', field: 'timezone' });
    ...
    expect(invalidCurrencyPayload.error).toMatchObject({ code: 'STARTUP_CURRENCY_INVALID', field: 'currency' });
    ...
    expect(invalidTypePayload.error).toMatchObject({ code: 'STARTUP_TYPE_INVALID', field: 'type' });
  });
  ...
  test('create and list startups within the active workspace, preventing repeated submissions from duplicating state', async () => {
    ...
    expect(createPayload.workspace).toMatchObject({ id: workspace.id, name: 'Acme Labs' });
    ...
    expect(duplicatePayload.error).toMatchObject({ code: 'STARTUP_ALREADY_EXISTS' });

    const listResponse = await send('/api/startups', { cookie });
    const listPayload = await parseJson(listResponse);

    expect(listResponse.status).toBe(200);
    expect(listPayload.workspace).toMatchObject({ id: workspace.id, name: 'Acme Labs' });
  ...
  test('isolate startup reads and writes per active workspace and fail closed on cross-workspace access attempts', async () => {
    const { cookie } = await createAuthenticatedSession('multi-workspace@example.com');
    ...
    expect(secondListPayload.workspace).toMatchObject({ id: secondWorkspace.id, name: 'Beta Labs' });
    ...
    expect(secondCreatePayload.startup).toMatchObject({ workspaceId: secondWorkspace.id, name: 'Beta Analytics' });
    ...
    expect(firstListPayload.workspace).toMatchObject({ id: firstWorkspace.id, name: 'Acme Labs' });
    ...
    expect(outsiderSetActivePayload.error).toMatchObject({ code: 'WORKSPACE_NOT_FOUND' });

    const outsiderStartupList = await send('/api/startups', { cookie: outsider.cookie });
    const outsiderStartupListPayload = await parseJson(outsiderStartupList);

    expect(outsiderStartupList.status).toBe(409);
    expect(outsiderStartupListPayload.error).toMatchObject({ code: 'ACTIVE_WORKSPACE_REQUIRED' });
  });
});
