...
async function listConnectors(app: ApiApp, cookie: string, startupId: string): Promise<Response> {
  return makeRequest(app, `http://localhost:3000/api/connectors?startupId=${encodeURIComponent(startupId)}`, {
    headers: { cookie },
  });
}

async function getConnectorStatus(app: ApiApp, cookie: string, connectorId: string): Promise<Response> {
  return makeRequest(app, `http://localhost:3000/api/connectors/${encodeURIComponent(connectorId)}/status`, {
    headers: { cookie },
  });
}

async function triggerSyncReq(app: ApiApp, cookie: string, connectorId: string): Promise<Response> {
  return makeRequest(app, `http://localhost:3000/api/connectors/${encodeURIComponent(connectorId)}/sync`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', cookie },
  });
}

async function deleteConnector(app: ApiApp, cookie: string, connectorId: string): Promise<Response> {
  return makeRequest(app, `http://localhost:3000/api/connectors/${encodeURIComponent(connectorId)}`, {
    method: 'DELETE',
    headers: { cookie },
  });
}
...
describe('connector integration lifecycle', () => {
  ...
  const queueProducer = createStubQueueProducer({ success: true, jobId: 'int-job-id' });
  ...
  beforeAll(async () => {
    ...
    startupId = await createStartup(app, cookie, { ...VALID_STARTUP, name: `Int Startup ${runId}` });
  });
  ...
  test('full happy path: create PostHog → list → status → manual resync', async () => {
    ...
    const createRes = await createConnector(app, cookie, startupId, 'posthog', POSTHOG_CONFIG);
    ...
    const listPayload = (await listRes.json()) as { connectors: ConnectorSummary[] };
    ...
    const syncPayload = (await syncRes.json()) as { syncJob: { trigger: string } };
    expect(syncPayload.syncJob.trigger).toBe('manual');
  });

  test('create Stripe → disconnect → resync rejected', async () => {
    const createRes = await createConnector(app, cookie, startupId, 'stripe', STRIPE_CONFIG);
    expect(createRes.status).toBe(201);

    const createPayload = (await createRes.json()) as { connector: ConnectorSummary };
    const connectorId = createPayload.connector.id;
  ...
  test('duplicate provider is rejected with 409', async () => {
    ...
    const res = await createConnector(app, cookie, startupId, 'posthog', POSTHOG_CONFIG);
  ...
  test('listing connectors after disconnect shows disconnected status', async () => {
    const listRes = await listConnectors(app, cookie, startupId);
    expect(listRes.status).toBe(200);
    const payload = (await listRes.json()) as { connectors: ConnectorSummary[] };
...
describe('connector integration — failure paths', () => {
  test('provider validation failure returns 422 with error detail', async () => {
    ...
    const failingPH = createStubPostHogValidator({ valid: false, error: 'Invalid API key', retryable: false });
