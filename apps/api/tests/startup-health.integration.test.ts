...
describe('startup health integration', () => {
  ...
  beforeAll(async () => {
    const queueProducer = createStubQueueProducer({ success: true, jobId: 'health-int-job' });
    ...
    startupId = await createStartup(app, cookie, { ...VALID_STARTUP, name: `Health Startup ${runId}` });
  });
  ...
  test('health endpoint returns empty state when no snapshot exists', async () => {
    ...
    if (res.status === 200) {
      const body = (await res.json()) as { health: HealthSnapshotSummary | null };
      // No snapshot yet — should be null or have empty state
      expect(body.health === null || body.health?.healthState === 'blocked').toBeTruthy();
    }
  });
