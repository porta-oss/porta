...
async function cleanupTasks(db: ApiApp['runtime']['db']['db'], startupId: string) {
  await db.execute(
...
describe('POST /api/tasks', () => {
  ...
  test('creates a task from the first insight action', async () => {
    ...
    expect(body.task.description).toBe('Identify which customer segment is leaving.');
...
describe('GET /api/tasks', () => {
  // ── Auth rejection ──
  test('rejects unauthenticated requests with 401', async () => {
    const res = await makeRequest(app, `http://localhost:3000/api/tasks?startupId=${startupId}`);
    expect(res.status).toBe(401);
    const body = (await res.json()) as { error: { code: string } };
    expect(body.error.code).toBe('AUTH_REQUIRED');
  });
  ...
  test('returns empty list when startup has no tasks', async () => {
    ...
    const res = await makeRequest(app, `http://localhost:3000/api/tasks?startupId=${freshId}`, {
      headers: { cookie },
    });
    expect(res.status).toBe(200);
  ...
  // ── Startup not found ──
  test('returns 404 for non-existent startup', async () => {
    const res = await makeRequest(app, 'http://localhost:3000/api/tasks?startupId=nonexistent', {
      headers: { cookie },
    });
    expect(res.status).toBe(404);
