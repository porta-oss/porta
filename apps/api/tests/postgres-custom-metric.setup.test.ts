...
describe('postgres custom metric setup', () => {
  describe('authentication and authorization', () => {
    ...
    test('reject cross-workspace startup access for postgres setup', async () => {
      ...
      const { cookie: cookieB } = await createAuthenticatedSession('pgB@example.com');
  ...
  describe('input validation — malformed inputs', () => {
    test('reject non-postgres URL scheme', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('pg-bad-scheme@example.com');
      ...
      const response = await send('/api/connectors', {
        ...
        body: {
          startupId: startup.id,
          provider: 'postgres',
          config: { ...VALID_POSTGRES_CONFIG, connectionUri: 'mysql://host/db' },
        },
      });
    ...
    test('reject unsafe schema identifier (SQL injection attempt)', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('pg-bad-schema@example.com');
      ...
      const response = await send('/api/connectors', {
        ...
        body: {
          startupId: startup.id,
          provider: 'postgres',
          config: { ...VALID_POSTGRES_CONFIG, schema: 'public; DROP TABLE users--' },
        },
      });
    ...
    test('reject unsafe view identifier', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('pg-bad-view@example.com');
    ...
    test('reject blank label', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('pg-blank-label@example.com');
    ...
    test('reject invalid unit (blank)', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('pg-blank-unit@example.com');
    ...
    test('reject missing connection URI', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('pg-no-uri@example.com');
      ...
      const response = await send('/api/connectors', {
        ...
        body: {
          startupId: startup.id,
          provider: 'postgres',
          config: { schema: 'public', view: 'test_view', label: 'Test', unit: '$' },
        },
      });
  ...
  describe('successful postgres setup', () => {
    test('create postgres connector with custom metric', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('pg-success@example.com');
      ...
      // Sync job queued
      expect(payload.syncJob).toMatchObject({ status: 'queued', trigger: 'initial' });
    ...
    test('credential redaction — connection URI never in response', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('pg-redact@example.com');
    ...
    test('list connectors includes custom metrics', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('pg-list@example.com');
  ...
  describe('duplicate prevention', () => {
    test('reject duplicate postgres setup for same startup', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('pg-dup@example.com');

      // First creation succeeds
      const first = await send('/api/connectors', {
        method: 'POST',
        cookie,
        body: { startupId: startup.id, provider: 'postgres', config: VALID_POSTGRES_CONFIG },
      });
  ...
  describe('coexistence with PostHog and Stripe', () => {
    test('startup can have posthog, stripe, and postgres connectors', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('pg-coexist@example.com');
  ...
  describe('queue failure handling', () => {
    test('surface queue enqueue failure without corrupting connector or metric state', async () => {
      ...
      try {
        ...
        const { cookie, startup } = await setupOnApp(failApp, 'pg-qfail@example.com');
        ...
        expect((payload.connector as Record<string, unknown>).provider).toBe('postgres');

        // Custom metric created successfully
        expect(payload.customMetric).toBeDefined();
        expect((payload.customMetric as Record<string, unknown>).label).toBe('Daily Revenue');

        // But sync job reflects the failure
        expect((payload.syncJob as Record<string, unknown>).status).toBe('failed');
      } finally {
...
async function setupOnApp(testApp: ApiApp, email: string) {
  ...
  return { cookie, startup: startupPayload.startup as { id: string; name: string; workspaceId: string } };
}
