...
async function sendOnApp(testApp: ApiApp, path: string, init?: { method?: string; body?: unknown; cookie?: string }) {
...
async function createWorkspaceOnApp(testApp: ApiApp, cookie: string, name: string) {
  const response = await sendOnApp(testApp, '/api/workspaces', { method: 'POST', cookie, body: { name } });
...
describe('founder-proof env parsing', () => {
  ...
  test('rejects malformed FOUNDER_PROOF_MODE values', () => {
    expect(() => readApiEnv({ ...BASE_ENV, FOUNDER_PROOF_MODE: 'yes' })).toThrow(
      /FOUNDER_PROOF_MODE must be one of/
    );
    expect(() => readApiEnv({ ...BASE_ENV, FOUNDER_PROOF_MODE: 'enabled' })).toThrow(
      /FOUNDER_PROOF_MODE must be one of/
    );
    expect(() => readApiEnv({ ...BASE_ENV, FOUNDER_PROOF_MODE: 'maybe' })).toThrow(
      /FOUNDER_PROOF_MODE must be one of/
    );
  });
});
...
describe('founder-proof mode off (default)', () => {
  ...
  test('/api/health reports founderProofMode=false and validationMode=live', async () => {
    ...
    expect((payload.connectors as Record<string, unknown>).validationMode).toBe('live');
  });

  test('PostHog demo credentials fail when proof mode is off', async () => {
    const { cookie, startup } = await setupWorkspaceAndStartupOnApp(app, 'proof-off-ph@example.com');
  ...
  test('Stripe demo credentials fail when proof mode is off', async () => {
    const { cookie, startup } = await setupWorkspaceAndStartupOnApp(app, 'proof-off-st@example.com');
...
describe('founder-proof mode on', () => {
  ...
  beforeAll(async () => {
    ...
    app = await createApiApp({ ...BASE_ENV, FOUNDER_PROOF_MODE: 'true' }, { queueProducer });
  });
  ...
  test('/api/health reports founderProofMode=true and validationMode=founder-proof', async () => {
    ...
    expect((payload.connectors as Record<string, unknown>).validationMode).toBe('founder-proof');
  });
  ...
  test('PostHog demo credentials succeed in proof mode', async () => {
    const { cookie, startup } = await setupWorkspaceAndStartupOnApp(app, 'proof-on-ph@example.com');
    ...
    expect(response.status).toBe(201);
    const payload = await parseJson(response);
    expect((payload.connector as Record<string, unknown>).provider).toBe('posthog');
    expect((payload.connector as Record<string, unknown>).status).toBe('pending');
  ...
  test('Stripe demo credentials succeed in proof mode', async () => {
    const { cookie, startup } = await setupWorkspaceAndStartupOnApp(app, 'proof-on-st@example.com');
    ...
    expect(response.status).toBe(201);
    const payload = await parseJson(response);
    expect((payload.connector as Record<string, unknown>).provider).toBe('stripe');
  ...
  test('both PostHog and Stripe can be connected for the same startup in proof mode', async () => {
    const { cookie, startup } = await setupWorkspaceAndStartupOnApp(app, 'proof-both@example.com');

    const phResponse = await sendOnApp(app, '/api/connectors', {
      method: 'POST',
      cookie,
      body: { startupId: startup.id, provider: 'posthog', config: POSTHOG_DEMO_CONFIG }
    });
    ...
    const stResponse = await sendOnApp(app, '/api/connectors', {
      method: 'POST',
      cookie,
      body: { startupId: startup.id, provider: 'stripe', config: STRIPE_DEMO_CONFIG }
    });
    expect(stResponse.status).toBe(201);

    const listResponse = await sendOnApp(app, `/api/connectors?startupId=${startup.id}`, { cookie });
  ...
  test('workspace scoping still works in proof mode', async () => {
    ...
    const { startup } = await setupWorkspaceAndStartupOnApp(app, 'proof-scope-a@example.com');

    // User B with their own workspace
    const { cookie: cookieB } = await createSessionOnApp(app, 'proof-scope-b@example.com');
...
describe('founder-proof mode negative tests', () => {
  ...
  test('blank PostHog fields rejected even in proof mode', async () => {
    const { cookie, startup } = await setupWorkspaceAndStartupOnApp(app, 'proof-blank-ph@example.com');
  ...
  test('blank Stripe key rejected even in proof mode', async () => {
    const { cookie, startup } = await setupWorkspaceAndStartupOnApp(app, 'proof-blank-st@example.com');

    const response = await sendOnApp(app, '/api/connectors', {
      method: 'POST',
      cookie,
      body: { startupId: startup.id, provider: 'stripe', config: { secretKey: '' } }
    });
    expect(response.status).toBe(422);
  });

  test('non-demo PostHog credentials rejected in proof mode', async () => {
    const { cookie, startup } = await setupWorkspaceAndStartupOnApp(app, 'proof-nondemo-ph@example.com');
    ...
    const response = await sendOnApp(app, '/api/connectors', {
      ...
      body: {
        startupId: startup.id,
        provider: 'posthog',
        config: { apiKey: 'phx_real_key', projectId: '99999', host: 'https://app.posthog.com' }
      }
    });
    expect(response.status).toBe(422);
    const payload = await parseJson(response);
    expect((payload.error as Record<string, unknown>).message).toContain('deterministic demo credentials');
  });

  test('non-demo Stripe key rejected in proof mode', async () => {
    const { cookie, startup } = await setupWorkspaceAndStartupOnApp(app, 'proof-nondemo-st@example.com');

    const response = await sendOnApp(app, '/api/connectors', {
      method: 'POST',
      cookie,
      body: { startupId: startup.id, provider: 'stripe', config: { secretKey: 'sk_test_real_key_value' } }
    });
    expect(response.status).toBe(422);
    const payload = await parseJson(response);
    expect((payload.error as Record<string, unknown>).message).toContain('deterministic demo');
  });

  test('unsupported provider still rejected in proof mode', async () => {
    const { cookie, startup } = await setupWorkspaceAndStartupOnApp(app, 'proof-unsupported@example.com');
  ...
  test('invalid Stripe key format still rejected in proof mode', async () => {
    const { cookie, startup } = await setupWorkspaceAndStartupOnApp(app, 'proof-badformat@example.com');

    const response = await sendOnApp(app, '/api/connectors', {
      method: 'POST',
      cookie,
      body: { startupId: startup.id, provider: 'stripe', config: { secretKey: 'not-a-stripe-key' } }
    });
    expect(response.status).toBe(422);
    const payload = await parseJson(response);
    expect((payload.error as Record<string, unknown>).message).toContain('key format is invalid');
  });
});
