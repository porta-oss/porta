...
import { createStubQueueProducer, createFailingQueueProducer } from '../src/lib/connectors/queue';
...
beforeAll(async () => {
  ...
  queueProducer = createStubQueueProducer({ success: true, jobId: 'stub-job-id' });
...
async function createStartup(cookie: string, overrides?: Partial<StartupDraft>) {
...
describe('connector routes', () => {
  describe('authentication and authorization', () => {
    test('reject unauthenticated connector access', async () => {
      ...
      const createResponse = await send('/api/connectors', {
        method: 'POST',
        body: { startupId: 'fake', provider: 'posthog', config: POSTHOG_CONFIG }
      });
      expect(createResponse.status).toBe(401);

      const deleteResponse = await send('/api/connectors/fake-id', { method: 'DELETE' });
      expect(deleteResponse.status).toBe(401);

      const syncResponse = await send('/api/connectors/fake-id/sync', { method: 'POST' });
    ...
    test('reject cross-workspace startup access for connector creation', async () => {
      ...
      const { cookie: cookieB } = await createAuthenticatedSession('userB@example.com');
      ...
      const response = await send('/api/connectors', {
        method: 'POST',
        cookie: cookieB,
        body: { startupId: startup.id, provider: 'posthog', config: POSTHOG_CONFIG }
      });
  ...
  describe('input validation', () => {
    test('reject missing startupId on list', async () => {
      const { cookie } = await setupWorkspaceAndStartup('list-no-startup@example.com');
    ...
    test('reject unsupported provider', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('bad-provider@example.com');
    ...
    test('reject malformed PostHog config', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('bad-posthog@example.com');
      ...
      const response = await send('/api/connectors', {
        ...
        body: {
          ...
          provider: 'posthog',
          config: { apiKey: 'phx_test_123', projectId: '', host: 'https://app.posthog.com' }
        }
      });
    ...
    test('reject malformed Stripe key', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('bad-stripe@example.com');
    ...
    test('reject blank PostHog host URL', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('bad-host@example.com');
      ...
      const response = await send('/api/connectors', {
        ...
        body: {
          startupId: startup.id,
          provider: 'posthog',
          config: { apiKey: 'phx_test_123', projectId: '12345', host: 'not-a-url' }
        }
      });
  ...
  describe('connector CRUD lifecycle', () => {
    test('create PostHog connector, list, get status, and delete', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('posthog-lifecycle@example.com');
      ...
      // List
      const listResponse = await send(`/api/connectors?startupId=${startup.id}`, { cookie });
      ...
      // Status
      const statusResponse = await send(`/api/connectors/${created.id}/status`, { cookie });
      expect(statusResponse.status).toBe(200);
      const statusPayload = await parseJson(statusResponse);
      expect(statusPayload.connector).toMatchObject({ id: created.id, provider: 'posthog' });
      ...
      const afterDeleteStatus = await send(`/api/connectors/${created.id}/status`, { cookie });
      const afterDeletePayload = await parseJson(afterDeleteStatus);
      expect((afterDeletePayload.connector as Record<string, unknown>).status).toBe('disconnected');
    });

    test('create Stripe connector with proper validation', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('stripe-lifecycle@example.com');

      const createResponse = await send('/api/connectors', {
        method: 'POST',
        cookie,
        body: { startupId: startup.id, provider: 'stripe', config: STRIPE_CONFIG }
      });
  ...
  describe('duplicate prevention', () => {
    test('reject duplicate provider for same startup', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('dup-provider@example.com');
    ...
    test('allow different providers for the same startup', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('multi-provider@example.com');
      ...
      const list = await send(`/api/connectors?startupId=${startup.id}`, { cookie });
  ...
  describe('provider validation failures', () => {
    test('reject connector creation when PostHog validation fails', async () => {
      const failingPosthog = createStubPostHogValidator({
        valid: false,
        error: 'PostHog API key is invalid or lacks access to the specified project.'
      });
      ...
      try {
        ...
        const { cookie: fCookie } = await createSessionOnApp(failApp, 'ph-fail@example.com');
        ...
        const list = await failApp.handle(
          new Request(`http://localhost/api/connectors?startupId=${startup.id}`, {
  ...
  describe('sync trigger and queue handoff', () => {
    test('manual sync enqueues reference-only job', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('manual-sync@example.com');
      ...
      const createPayload = await parseJson(createResponse);
      const connectorId = (createPayload.connector as Record<string, unknown>).id as string;
      ...
      const lastCall = queueProducer.calls[queueProducer.calls.length - 1]!;
    ...
    test('reject sync on disconnected connector', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('sync-disconnected@example.com');
      ...
      // Disconnect
      await send(`/api/connectors/${connectorId}`, { method: 'DELETE', cookie });
    ...
    test('surface queue enqueue failure without corrupting connector state', async () => {
      ...
      try {
        ...
        const { cookie } = await createSessionOnApp(failApp, 'queue-fail@example.com');
        ...
        expect((createPayload.syncJob as Record<string, unknown>).status).toBe('failed');
  ...
  describe('status and history reads', () => {
    test('return empty sync history for first connector', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('empty-history@example.com');
      ...
      const statusResponse = await send(`/api/connectors/${connectorId}/status`, { cookie });
      expect(statusResponse.status).toBe(200);
      const statusPayload = await parseJson(statusResponse);
    ...
    test('return 404 for non-existent connector status', async () => {
      const { cookie } = await setupWorkspaceAndStartup('no-connector@example.com');

      const response = await send('/api/connectors/non-existent-id/status', { cookie });
    ...
    test('empty connector list for startup with no connectors', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('no-connectors@example.com');

      const response = await send(`/api/connectors?startupId=${startup.id}`, { cookie });
  ...
  describe('secret redaction', () => {
    test('no raw credentials in any API response', async () => {
      const { cookie, startup } = await setupWorkspaceAndStartup('redaction@example.com');
      ...
      const connectorId = (phPayload.connector as Record<string, unknown>).id as string;
      const statusResponse = await send(`/api/connectors/${connectorId}/status`, { cookie });
...
async function createWorkspaceOnApp(testApp: ApiApp, cookie: string, name: string) {
...
async function createStartupOnApp(testApp: ApiApp, cookie: string, overrides?: Partial<StartupDraft>) {
  const response = await testApp.handle(
