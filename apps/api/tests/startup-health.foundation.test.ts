...
function makeSnapshotInput(startupId: string, overrides?: Partial<ReplaceSnapshotInput>): ReplaceSnapshotInput {
...
describe('shared contract — enum guards', () => {
  ...
  test('supporting metric labels and units cover all keys', () => {
    for (const key of SUPPORTING_METRICS) {
      ...
      expect(['count', 'currency', 'percent']).toContain(SUPPORTING_METRIC_UNITS[key]);
    }
  });
...
describe('shared contract — validation helpers', () => {
  ...
  test('validateSupportingMetrics rejects missing key', () => {
    ...
    expect(validateSupportingMetrics(rest)).toContain('Missing supporting metric key: active_users');
  });

  test('validateSupportingMetrics rejects unknown key', () => {
    const metrics = { ...emptySupportingMetrics(), bogus_key: { value: 0, previous: null } };
    expect(validateSupportingMetrics(metrics)).toContain('Unknown supporting metric key: bogus_key');
  });

  test('validateSupportingMetrics rejects non-finite value', () => {
    const metrics = emptySupportingMetrics();
    metrics.churn_rate = { value: NaN, previous: null };
  ...
  test('validateFunnelStages rejects missing stage', () => {
    ...
    expect(validateFunnelStages(stages)).toContain('Missing funnel stage: visitor');
  });
  ...
  test('validateFunnelStages rejects invalid stage name', () => {
    const stages = emptyFunnelStages().map((s) =>
      s.stage === 'visitor' ? { ...s, stage: 'bogus' as FunnelStageRow['stage'] } : s,
    );
  ...
  test('validateFunnelStages rejects non-finite value', () => {
    const stages = emptyFunnelStages().map((s) =>
      s.stage === 'visitor' ? { ...s, value: Infinity } : s,
    );
...
describe('snapshot persistence', () => {
  test('first snapshot for a startup can be written and read back', async () => {
    ...
    expect(snapshot!.northStarValue).toBe(12500);
    expect(snapshot!.northStarPreviousValue).toBe(10000);
    ...
    expect(funnel[funnel.length - 1]!.stage).toBe('paying_customer');
  });
  ...
  test('replacing an existing snapshot atomically swaps all data', async () => {
    ...
    const snapshot = await healthRepo.findSnapshot(startupId);
    expect(snapshot).toBeDefined();
    expect(snapshot!.northStarValue).toBe(15000);
...
describe('negative tests — malformed inputs', () => {
  ...
  test('validateSupportingMetrics rejects entry with non-number previous', () => {
    const metrics = emptySupportingMetrics();
    (metrics.arpu as unknown as Record<string, unknown>).previous = 'not-a-number';
