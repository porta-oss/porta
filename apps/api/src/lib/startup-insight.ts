...
function computeDisplayStatus(
  row: InsightRow | null,
): { displayStatus: InsightDisplayStatus; diagnosticMessage: string | null } {
  if (!row) {
    return {
      displayStatus: 'unavailable',
      diagnosticMessage: 'No insight has been generated for this startup yet. Insights are created after the first data sync completes.',
    };
  }
  ...
  if (status === 'skipped_blocked' || status === 'skipped_stale') {
    return {
      ...
      diagnosticMessage: status === 'skipped_blocked'
  ...
  if (status === 'skipped_no_condition') {
    ...
    return {
      displayStatus: 'unavailable',
      diagnosticMessage: 'No actionable condition was detected in the latest data. Insights will appear when a condition is identified.',
    };
  }
  ...
  if (status === 'failed_explainer' || status === 'failed_persistence') {
    ...
    return {
      displayStatus: 'error',
      diagnosticMessage: row.last_error ?? `Insight generation failed with status: ${status}.`,
    };
  }
