 ...
 */
export function createPostHogValidator(options?: { timeoutMs?: number }): PostHogValidator {
    ...
    async validate(config: PostHogConfig): Promise<ProviderValidationResult> {
      ...
      try {
        ...
        if (err instanceof DOMException && err.name === 'AbortError') {
          return {
            valid: false,
            error: 'PostHog validation timed out. Check the host URL and try again.',
            retryable: true
          };
        }
        ...
        return {
          valid: false,
          error: 'PostHog validation request failed. Check the host URL and network connectivity.',
          retryable: true
        };
      }
    }
...
export function createFounderProofPostHogValidator(): PostHogValidator {
  return {
    async validate(config: PostHogConfig): Promise<ProviderValidationResult> {
      ...
      return {
        valid: false,
        error: 'Founder-proof mode requires the deterministic demo credentials. Use the documented proof values.'
      };
    }
  };
}

export { POSTHOG_DEMO_API_KEY, POSTHOG_DEMO_PROJECT_ID, POSTHOG_DEMO_HOST };
