...
export const connector = pgTable(
  ...
  (table) => [
    uniqueIndex('connector_startup_provider_uidx').on(table.startupId, table.provider),
    index('connector_startupId_idx').on(table.startupId),
    index('connector_status_idx').on(table.status),
    check('connector_provider_check', sql`${table.provider} IN ('posthog', 'stripe', 'postgres')`),
    check(
...
export const syncJob = pgTable(
  ...
  (table) => [
    ...
    check('sync_job_status_check', sql`${table.status} IN ('queued', 'running', 'completed', 'failed')`),
    check('sync_job_trigger_check', sql`${table.trigger} IN ('initial', 'manual', 'scheduled')`)
  ]
);
