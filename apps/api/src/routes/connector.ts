...
import type { CustomMetricSummary, PostgresSetupInput } from '@shared/custom-metric';
import { encryptConnectorConfig, parseEncryptionKey } from '@shared/crypto';

import { connector } from '../db/schema/connector';
import { syncJob } from '../db/schema/connector';
import { customMetric } from '../db/schema/custom-metric';

import type { PostHogConfig, PostHogValidator, ProviderValidationResult } from '../lib/connectors/posthog';
...
function getPgErrorCode(error: unknown): string | undefined {
  ...
  if (isRecord(error) && 'cause' in error) return getPgErrorCode(error.cause);
  return undefined;
}
...
async function verifyStartupOwnership(
  ...
  const startupTable = await import('../db/schema/startup').then((m) => m.startup);
  const rows = await db
    .select({ id: startupTable.id })
    .from(startupTable)
    .where(and(eq(startupTable.id, startupId), eq(startupTable.workspaceId, workspaceId)));
  return rows[0] ?? null;
}
...
export async function handleListConnectors(
  ...
  if (!startupId) {
    set.status = 400;
    return {
      error: { code: 'STARTUP_ID_REQUIRED', message: 'startupId query parameter is required.' }
    };
  }

  const startup = await verifyStartupOwnership(runtime.db.db, startupId, wsCtx.workspace.id);
  if (!startup) {
    set.status = 403;
    return {
      error: {
...
export async function handleCreateConnector(
  ...
  const validation = await validateProviderConfig(runtime, provider, config ?? {});
  if (!validation.valid) {
    set.status = 422;
...
export async function handleDeleteConnector(
  ...
  const startup = await verifyStartupOwnership(runtime.db.db, existing.startupId, wsCtx.workspace.id);
  if (!startup) {
    set.status = 403;
    return {
      error: {
        code: 'STARTUP_SCOPE_INVALID',
        message: 'The connector does not belong to a startup in the active workspace.'
      }
    };
  }
...
export async function handleTriggerSync(
  ...
  const runningJobs = await runtime.db.db
    ...
    .where(
      and(
        eq(syncJob.connectorId, connectorId),
  ...
  if (runningJobs.length > 0) {
    ...
    return {
      error: {
        ...
        message: 'A sync is already running for this connector. Wait for it to complete.'
      }
    };
  }
  ...
  if (!enqueueResult.success) {
    ...
    return {
      error: {
        ...
        message: 'Failed to enqueue sync job. The connector status has not changed.',
        retryable: true
      }
    };
  }
