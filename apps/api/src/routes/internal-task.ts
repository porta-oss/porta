...
import { internalTask } from '../db/schema/internal-task';
...
async function verifyStartupOwnership(
  ...
  const row = result.rows[0] as { id: string; workspace_id: string } | undefined;
...
export async function handleCreateTask(
  ...
  if (
    ...
    set.status = 400;
    console.warn('[internal-task] invalid actionIndex', { startupId, actionIndex });
    return {
      error: {
  ...
  const ownership = await verifyStartupOwnership(db, startupId, ctx.workspace.id);
  ...
  if (!insightRow) {
    ...
    return {
      error: {
        ...
        message: 'No insight has been generated for this startup yet. Generate an insight first.',
      },
    };
  }
  ...
  if (!explanation || !Array.isArray(explanation.actions)) {
    ...
    return {
      error: {
        ...
        message: 'The latest insight has no explanation or actions. A successful insight generation is required.',
      },
    };
  }
  ...
  if (!createdRow) {
    set.status = 502;
    console.error('[internal-task] create returned no rows', { startupId, actionIndex });
    return {
      error: {
