...
function createSyncingPayload(): StartupHealthPayload {
  return {
    health: null,
    connectors: [
      { provider: 'posthog', status: 'pending', lastSyncAt: null, lastSyncError: null },
    ],
    status: 'syncing',
    blockedReasons: [],
    lastSnapshotAt: null,
...
describe('portfolio startup card', () => {
  test('renders a healthy portfolio card with name, badge, trend, freshness, and top-issue', async () => {
    ...
    // Name
    expect(view.getByTestId('portfolio-startup-name').textContent).toContain('Acme Analytics');
  ...
  test('renders an error portfolio card with error message when health fetch fails', async () => {
    ...
    expect(view.getByRole('button', { name: 'Retry health load' })).toBeTruthy();
  });
  ...
  test('shows loading portfolio card while health data is being fetched', async () => {
    ...
    await waitFor(() => {
      expect(view.getByTestId('portfolio-badge').textContent).toContain('Healthy');
    });
  });
  ...
  test('handles malformed health payload as a recoverable portfolio-card error', async () => {
    const api = createApi({
      fetchHealth: mock(async () => {
        throw new Error('Health snapshot contains invalid data: Missing supporting metric key: arpu.');
      }),
    });
    const view = render(<DashboardPage authState={createAuthenticatedSnapshot()} api={api} />);
    ...
    const topIssue = view.getByTestId('portfolio-top-issue');
    expect(topIssue.textContent).toContain('Health snapshot contains invalid data');
  });
  ...
  test('does not render portfolio card when no startups exist', async () => {
    ...
    expect((await view.findAllByText('No startups are attached to this workspace yet.')).length).toBeGreaterThanOrEqual(1);
  ...
  test('portfolio card shows connector sync error as top issue in error state', async () => {
    const payload: StartupHealthPayload = {
      health: null,
      connectors: [
        { provider: 'stripe', status: 'error', lastSyncAt: null, lastSyncError: 'Stripe API key is invalid' },
      ],
      status: 'error',
      blockedReasons: [],
      lastSnapshotAt: null,
