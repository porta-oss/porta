...
function createCustomMetric(overrides: Partial<CustomMetricSummary> = {}): CustomMetricSummary {
  return {
    id: 'cm_1',
    startupId: `${WORKSPACE_A.id}_Acme Analytics`,
...
function createHealthyPayload(customMetric: CustomMetricSummary | null = null): StartupHealthPayload {
  return {
    health: {
      startupId: `${WORKSPACE_A.id}_Acme Analytics`,
      healthState: 'ready',
...
describe('postgres custom metric on dashboard', () => {
  test('shows the Postgres custom metric setup form when no postgres connector exists', async () => {
    ...
    expect(view.getByLabelText('Postgres custom metric setup form')).toBeTruthy();
    expect(view.getByRole('button', { name: 'Add Postgres metric' })).toBeTruthy();
  });
  ...
  test('renders the synced custom metric value with delta beneath the fixed health template', async () => {
    ...
    const api = createApi({
      ...
      listConnectors: mock(async () => ({ connectors: [createConnector('posthog'), createConnector('stripe'), createConnector('postgres')] })),
    });
    const view = render(<DashboardPage authState={createAuthenticatedSnapshot()} api={api} />);
  ...
  test('shows error state in custom metric panel when metric has error status', async () => {
    const cm = createCustomMetric({ status: 'error', metricValue: 4250, previousValue: 3900 });
    const api = createApi({
      fetchHealth: mock(async () => createHealthyPayload(cm)),
      listConnectors: mock(async () => ({ connectors: [createConnector('posthog'), createConnector('stripe'), createConnector('postgres', 'error')] })),
    });
    const view = render(<DashboardPage authState={createAuthenticatedSnapshot()} api={api} />);
  ...
  test('shows pending state when custom metric is waiting for first sync', async () => {
    const cm = createCustomMetric({ status: 'pending', metricValue: null, previousValue: null, capturedAt: null });
    const api = createApi({
      fetchHealth: mock(async () => createHealthyPayload(cm)),
      listConnectors: mock(async () => ({ connectors: [createConnector('posthog'), createConnector('stripe'), createConnector('postgres', 'pending')] })),
    });
    const view = render(<DashboardPage authState={createAuthenticatedSnapshot()} api={api} />);
  ...
  test('validates blank fields before submitting postgres setup', async () => {
    ...
    expect(await view.findByTestId('postgres-custom-metric-setup')).toBeTruthy();

    // Submit with empty fields
    fireEvent.submit(view.getByLabelText('Postgres custom metric setup form'));

    const alerts = await view.findAllByRole('alert');
    expect(alerts.some((a) => a.textContent?.includes('Connection URI is required'))).toBe(true);
  ...
  test('validates connection URI scheme before submitting', async () => {
    ...
    setNativeInputValue(view.getByLabelText('Connection URI') as HTMLInputElement, 'mysql://bad:uri@host/db');
    setNativeInputValue(view.getByLabelText('View') as HTMLInputElement, 'daily_revenue');
    setNativeInputValue(view.getByLabelText('Label') as HTMLInputElement, 'Daily Revenue');
    setNativeInputValue(view.getByLabelText('Unit') as HTMLInputElement, '$');

    fireEvent.submit(view.getByLabelText('Postgres custom metric setup form'));

    const alerts = await view.findAllByRole('alert');
    expect(alerts.some((a) => a.textContent?.includes('postgres://'))).toBe(true);
    expect(createPostgresMetric).not.toHaveBeenCalled();
  });
  ...
  test('validates blank label and unit before submitting', async () => {
    ...
    setNativeInputValue(view.getByLabelText('Connection URI') as HTMLInputElement, 'postgresql://user:pass@host:5432/db');
    setNativeInputValue(view.getByLabelText('View') as HTMLInputElement, 'daily_revenue');
    ...
    const alerts = await view.findAllByRole('alert');
    expect(alerts.some((a) => a.textContent?.includes('Label must not be blank'))).toBe(true);
    expect(createPostgresMetric).not.toHaveBeenCalled();
  });

  test('submits valid postgres setup and shows configured state', async () => {
    const newMetric = createCustomMetric({ status: 'pending', metricValue: null, previousValue: null, capturedAt: null });
    const createPostgresMetric = mock(async () => ({
      connector: createConnector('postgres', 'pending'),
    ...
    expect(await view.findByTestId('postgres-custom-metric-configured')).toBeTruthy();
    expect(view.queryByTestId('postgres-custom-metric-setup')).toBeNull();
  });
  ...
  test('shows setup error from API without losing form state', async () => {
    ...
    const alerts = await view.findAllByRole('alert');
    expect(alerts.some((a) => a.textContent?.includes('Duplicate postgres connector'))).toBe(true);

    // Form values should be preserved
    expect((view.getByLabelText('Connection URI') as HTMLInputElement).value).toBe('postgresql://user:pass@host:5432/db');
    expect((view.getByLabelText('Label') as HTMLInputElement).value).toBe('Daily Revenue');
  });
  ...
  test('custom metric panel preserves last-good value when health refresh fails', async () => {
    ...
    const fetchHealth = mock(async () => {
      fetchCount += 1;
      if (fetchCount <= 1) return createHealthyPayload(cm);
    ...
    expect(view.getByTestId('custom-metric-value').textContent).toContain('4,250');
    ...
    expect(view.getByTestId('custom-metric-delta').textContent).toContain('+9.0%');
  });

  test('error-state custom metric shows no-data guidance when metric has never synced', async () => {
    const cm = createCustomMetric({ status: 'error', metricValue: null, previousValue: null, capturedAt: null });
    const api = createApi({
      fetchHealth: mock(async () => createHealthyPayload(cm)),
      listConnectors: mock(async () => ({ connectors: [createConnector('posthog'), createConnector('stripe'), createConnector('postgres', 'error')] })),
    });
    ...
    expect(panel.textContent).toContain('Check the Postgres connector configuration');
  });
});
