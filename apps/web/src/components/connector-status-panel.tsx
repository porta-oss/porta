...
export function ConnectorStatusPanel({
  ...
  const [actionStates, setActionStates] = useState<Record<string, 'idle' | 'working' | 'error'>>({});
  ...
  async function handleDisconnect(connectorId: string) {
    ...
    try {
      ...
      setActionErrors((s) => ({
        ...s,
        [connectorId]: err instanceof Error ? err.message : 'Disconnect failed.',
      }));
    }
  }
  ...
  return (
    ...
    >
      ...
      {loading && connectors.length === 0 ? (
        <p role="status" style={{ margin: 0 }}>Loading connectors…</p>
      ) : null}

      {error ? (
        <p role="alert" style={{ margin: 0, color: '#991b1b' }}>{error}</p>
      ) : null}
      ...
      {connectors.map((c) => {
        ...
        return (
          ...
          >
            ...
            {c.lastSyncError ? (
              <p role="alert" style={{ margin: 0, fontSize: '0.8rem', color: '#991b1b' }}>
                {c.lastSyncError}
              </p>
            ) : null}
