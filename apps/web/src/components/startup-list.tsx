...
export function StartupList({ workspaceName, startups, status, error = null, onRetry }: StartupListProps) {
  ...
  return (
    ...
    >
      ...
      {status === 'refreshing' ? <p role="status">Refreshing startup navigation for the active workspace…</p> : null}
      ...
      {status !== 'error' && !workspaceName ? (
        <div style={{ display: 'grid', gap: '0.5rem' }}>
          <p style={{ margin: 0 }}>The dashboard shell is ready, but startup navigation stays locked until a workspace becomes active.</p>
          <a href="/app/onboarding">Create or select a workspace in onboarding</a>
        </div>
      ) : null}

      {status !== 'error' && workspaceName && startups.length === 0 && !isBusy ? (
        <div style={{ display: 'grid', gap: '0.5rem' }}>
          ...
          {startups.map((startup, index) => (
            <li key={startup.id}>
              ...
              <div style={{ color: '#4b5563' }}>
                {index === 0 ? 'Primary startup' : 'Startup'} · {startup.stage.replace('_', ' ')} · {startup.type.replace('_', ' ')} · {startup.timezone} · {startup.currency}
              </div>
            </li>
          ))}
        </ul>
