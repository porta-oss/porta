...
function computeDelta(current: number | null, previous: number | null): string | null {
...
export function CustomMetricPanel({
    ...
    return (
      ...
      >
        <p style={{ margin: 0, fontSize: '0.85rem', color: '#6b7280' }}>
          No custom metric configured. Add a Postgres-backed metric below to track additional KPIs.
        </p>
      </section>
    );
  }
    ...
    return (
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <span role="status" style={{ fontSize: '0.8rem', fontWeight: 500, color: '#991b1b' }}>
            Sync failed
          </span>
        </div>
        {customMetric.metricValue !== null ? (
          <div data-testid="custom-metric-value">
            <p style={{ margin: 0, fontSize: '0.85rem', color: '#92400e' }}>
              Last known: {formatMetricValue(customMetric.metricValue, customMetric.unit)}
            </p>
          </div>
        ) : (
          <p style={{ margin: 0, fontSize: '0.85rem', color: '#991b1b' }}>
            No data has been synced yet. Check the Postgres connector configuration.
          </p>
        )}
      </section>
    );
  ...
  if (customMetric.status === 'pending') {
    return (
      ...
      >
        <p style={{ margin: 0, fontWeight: 600 }}>{customMetric.label}</p>
        <p role="status" style={{ margin: 0, fontSize: '0.85rem', color: '#92400e' }}>
  ...
  const delta = computeDelta(customMetric.metricValue, customMetric.previousValue);
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <p style={{ margin: 0, fontWeight: 600 }}>{customMetric.label}</p>
        <span role="status" style={{ fontSize: '0.8rem', fontWeight: 500, color: '#065f46' }}>
          Active
        </span>
      </div>
      <div data-testid="custom-metric-value" style={{ display: 'flex', alignItems: 'baseline', gap: '0.5rem' }}>
