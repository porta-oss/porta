...
import { INSIGHT_CONDITION_LABELS, type InsightConditionCode } from '@shared/startup-insight';
...
function EvidenceBullets({ insight }: { insight: LatestInsightPayload }) {
  ...
  return (
    <ul
      ...
      style={{ margin: '0.5rem 0', paddingLeft: '1.25rem', listStyleType: 'disc' }}
    >
      {items.map((item, i) => (
        <li key={`${item.metricKey}-${i}`} style={{ fontSize: '0.85rem', color: '#374151', marginBottom: '0.25rem' }}>
          <strong>{item.label}:</strong>{' '}
          {formatMetricValue(item.currentValue)}
          {item.previousValue !== null ? (
            <span style={{ color: item.direction === 'down' ? '#dc2626' : item.direction === 'up' ? '#16a34a' : '#6b7280' }}>
              {' '}({item.direction === 'down' ? '↓' : item.direction === 'up' ? '↑' : '→'} from {formatMetricValue(item.previousValue)})
            </span>
          ) : null}
        </li>
      ))}
...
function ExplanationSection({ explanation }: { explanation: InsightExplanation }) {
  return (
    <div style={{ display: 'grid', gap: '0.75rem' }}>
      <div data-testid="insight-observation">
        <p style={{ margin: 0, fontSize: '0.7rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: '#6b7280' }}>
          Observation
        </p>
        <p style={{ margin: '0.25rem 0 0', fontSize: '0.9rem', color: '#111827' }}>
...
function ActionList({ actions, tasks = [], creatingActionIndex = null, taskCreateError = null, onCreateTask, sourceInsightId }: ActionListProps) {
  ...
  return (
    <div data-testid="insight-actions">
      <p style={{ margin: '0 0 0.5rem', fontSize: '0.7rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: '#6b7280' }}>
        Recommended Actions
      </p>
        ...
        {actions.map((action, i) => {
          ...
          return (
            <li key={`action-${i}`} style={{ marginBottom: '0.75rem' }}>
              <strong style={{ fontSize: '0.9rem', color: '#111827' }}>{action.label}</strong>
                ...
                {existingTask ? (
                  <>
                    <span
                      ...
                      style={{ fontSize: '0.8rem', color: '#16a34a', fontWeight: 500 }}
                    >
                      ✓ Task created
                    </span>
      ...
      {taskCreateError ? (
        <p data-testid="task-create-error" role="alert" style={{ margin: '0.25rem 0 0', fontSize: '0.75rem', color: '#dc2626' }}>
          {taskCreateError}
        </p>
      ) : null}
    </div>
...
export function StartupInsightCard({
  ...
  if (displayStatus === 'unavailable') {
    return (
      ...
      >
        ...
        <p data-testid="insight-unavailable" style={{ margin: 0, color: '#6b7280', fontSize: '0.9rem' }}>
  ...
  if (displayStatus === 'blocked') {
    return (
      ...
      >
        <p style={{ margin: 0, fontSize: '0.75rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: '#92400e' }}>
          Grounded Insight — Blocked
        </p>
        <p data-testid="insight-blocked" role="status" style={{ margin: 0, color: '#92400e', fontSize: '0.9rem' }}>
  ...
  if (displayStatus === 'error') {
    return (
      ...
      >
        <p style={{ margin: 0, fontSize: '0.75rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: '#991b1b' }}>
          Grounded Insight — Error
        </p>
        <p data-testid="insight-error" role="alert" style={{ margin: 0, color: '#991b1b', fontSize: '0.9rem' }}>
          {diagnosticMessage ?? 'Failed to generate insight.'}
        </p>
        {onRetry ? (
          <button type="button" onClick={onRetry} style={{ justifySelf: 'start' }}>
            Retry insight load
          </button>
        ) : null}
      </section>
  ...
  if (!insight || !insight.explanation) {
  ...
  const conditionLabel = INSIGHT_CONDITION_LABELS[insight.conditionCode] ?? insight.conditionCode;
  ...
  return (
    ...
    >
      {/* Header */}
      <div>
        <p style={{ margin: 0, fontSize: '0.75rem', letterSpacing: '0.08em', textTransform: 'uppercase', color: '#1d4ed8' }}>
          Grounded Insight
        </p>
        <p data-testid="insight-condition" style={{ margin: '0.25rem 0 0', fontSize: '1rem', fontWeight: 600, color: '#1e3a5f' }}>
          {conditionLabel}
        </p>
      </div>
      ...
      {diagnosticMessage ? (
        <p data-testid="insight-diagnostic" style={{ margin: 0, fontSize: '0.8rem', color: '#92400e', fontStyle: 'italic' }}>
          {diagnosticMessage}
        </p>
      ) : null}
    </section>
