...
function computeDelta(current: number, previous: number | null): { label: string; direction: 'up' | 'down' | 'flat' } | null {
  if (previous === null || previous === 0) return null;
  const pct = ((current - previous) / previous) * 100;
  if (Math.abs(pct) < 0.01) return { label: '0%', direction: 'flat' };
  const sign = pct > 0 ? '+' : '';
  return {
...
function healthStateBanner(state: HealthState): { color: string; bg: string; text: string } {
  switch (state) {
    case 'ready':
        ...
        {delta ? (
          <span
            ...
            style={{
              fontSize: '0.85rem',
              fontWeight: 500,
              color: delta.direction === 'up' ? '#065f46' : delta.direction === 'down' ? '#991b1b' : '#6b7280',
            }}
          >
