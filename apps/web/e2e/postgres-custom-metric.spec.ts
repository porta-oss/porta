...
test.describe('postgres custom metric dashboard flow', () => {
  test('setup form is visible on dashboard after onboarding is complete', async ({ page }) => {
    // Navigate to dashboard (assumes authenticated session from global setup)
    await page.goto('/app');

    // Wait for dashboard to load
    await expect(page.getByRole('main', { name: 'dashboard shell' })).toBeVisible({ timeout: 10_000 });
    ...
    const hasStartups = await page.getByText(/Primary startup:/).isVisible().catch(() => false);

    if (hasStartups) {
      await expect(setupForm).toBeVisible({ timeout: 5_000 });
      await expect(page.getByRole('button', { name: 'Add Postgres metric' })).toBeVisible();
    }
  });
  ...
  test('setup form validates blank fields', async ({ page }) => {
    ...
    await expect(page.getByRole('alert').first()).toContainText('Connection URI is required');
  });

  test('custom metric panel shows not-configured guidance when no metric exists', async ({ page }) => {
    await page.goto('/app');
    await expect(page.getByRole('main', { name: 'dashboard shell' })).toBeVisible({ timeout: 10_000 });
  ...
  test('onboarding flow completes without postgres being required', async ({ page }) => {
    await page.goto('/app/onboarding');

    // Onboarding page should load
    await expect(page.getByRole('main', { name: 'startup onboarding' })).toBeVisible({ timeout: 10_000 });

    // Verify PostHog and Stripe are the only connector options in onboarding
    // Postgres should NOT appear in onboarding
    const onboardingText = await page.getByRole('main', { name: 'startup onboarding' }).textContent();
