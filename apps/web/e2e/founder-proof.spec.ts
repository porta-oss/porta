...
test.describe('founder self-serve proof', () => {
  test('completes the full founder journey: sign-in → onboarding → connectors → health → insight → task → reload', async ({
    ...
    await expect(page.getByLabel('startup list')).toBeVisible({ timeout: 10_000 });
    ...
    if (hasActions) {
      ...
      await expect(page.getByTestId('action-0-task-created')).toBeVisible({ timeout: 10_000 });

      // Task list section should appear with the new task
      await expect(page.getByTestId('startup-task-list')).toBeVisible({ timeout: 5_000 });
    ...
    await waitForHealthReady(page, 30_000);
    await expect(page.getByLabel('startup list').getByText(startupName)).toBeVisible({ timeout: 10_000 });
    await waitForInsightReady(page, 30_000);

    // If tasks were created, they should survive reload
    if (hasActions) {
      await expect(page.getByTestId('startup-task-list')).toBeVisible({ timeout: 10_000 });
