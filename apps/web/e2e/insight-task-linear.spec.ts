...
const MOCK_INSIGHT = {
  insight: {
    ...
    evidence: {
      conditionCode: 'mrr_declining',
      items: [{ metricKey: 'mrr', label: 'MRR', currentValue: 9500, previousValue: 11000, direction: 'down' }],
    ...
    explanation: {
      ...
      actions: [
        { label: 'Review churn cohorts', rationale: 'Identify leaving segments.' },
        { label: 'Run pricing experiment', rationale: 'Test alternative tiers.' },
      ],
...
test.describe('insight → task → Linear sync flow', () => {
  test('founder creates a task from an insight action and sees sync state', async ({ page }) => {
    ...
    await page.route('**/api/startups', (route) =>
      route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(MOCK_STARTUPS) }),
    );
    ...
    await page.route('**/api/startups/*/insight', (route) =>
      route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(MOCK_INSIGHT) }),
    );

    // Initially empty task list
    let taskList = { tasks: [], startupId: 'startup_1', count: 0 };
    await page.route('**/api/tasks?**', (route) =>
      route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(taskList) }),
    );
    ...
    await expect(page.getByTestId('startup-insight-card')).toBeVisible({ timeout: 10000 });
    ...
    await expect(page.getByTestId('action-0-task-created')).toBeVisible({ timeout: 5000 });
