...
test.describe('portfolio prioritization surface', () => {
  test('founder signs in, completes onboarding, and sees the portfolio card with health, trend, freshness, top issue — survives reload', async ({
    ...
    await expect(page.getByRole('heading', { name: 'Startup prioritization' })).toBeVisible();
    ...
    await expect(page.getByTestId('portfolio-startup-name')).toContainText(startupName);
    ...
    await expect(page.getByTestId('portfolio-startup-card')).toBeVisible({ timeout: 15_000 });
