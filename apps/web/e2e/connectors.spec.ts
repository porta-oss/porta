...
test.describe('connector setup and status flow', () => {
  test('founder connects PostHog and Stripe during onboarding, sees status on dashboard', async ({
    ...
    await expect(page.getByText('Connected').nth(1)).toBeVisible({ timeout: 10_000 });
    ...
    await expect(page.getByRole('main', { name: 'dashboard shell' })).toBeVisible();
    await expect(
      page.getByText(`Primary startup:`),
    ).toBeVisible();
    ...
    await page.reload();
    await expect(page).toHaveURL(/\/app$/);
    await expect(page.getByLabel('connector status')).toBeVisible({ timeout: 10_000 });
    await expect(page.getByLabel('connector status').getByText('PostHog')).toBeVisible();
    await expect(page.getByLabel('connector status').getByText('Stripe')).toBeVisible();
  });
  ...
  test('founder skips connectors during onboarding and adds them from dashboard', async ({
    ...
    await expect(page.getByRole('form', { name: 'PostHog setup form' })).toBeVisible({
      timeout: 10_000,
    });
    await expect(page.getByRole('form', { name: 'Stripe setup form' })).toBeVisible();
  ...
  test('client-side validation prevents blank PostHog API key submission', async ({ page }) => {
    ...
    await expect(posthogForm.getByRole('alert')).toContainText('cannot be blank');
  });
});
