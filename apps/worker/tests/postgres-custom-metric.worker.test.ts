...
import type { PostgresSyncResult } from "../src/providers";
import type {
...
function makePostgresConnectorRow(
...
describe("postgres custom metric sync — failure preserves last-good", () => {
  ...
  test("retryable timeout throws for BullMQ retry while preserving data", async () => {
    ...
    await expect(processor(makeFakeJob(makePayload()))).rejects.toThrow(
...
describe("postgres custom metric sync — boundary conditions", () => {
  test("custom metric upsert failure does not fail the sync job", async () => {
    ...
    const errorLog = log.entries.find(
      (e) => e.msg === "custom metric update failed — previous data preserved"
    );
  ...
  test("connector not found results in sync failure", async () => {
    ...
    const processor = createSyncProcessor({
      repo,
      encryptionKey: TEST_ENCRYPTION_KEY,
      validateProvider: createStubPostgresProvider({
        valid: true,
        mrr: null,
        supportingMetrics: null,
        funnelStages: null,
