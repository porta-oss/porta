...
function makeTaskRow(overrides?: Partial<InternalTaskRow>): InternalTaskRow {
  return {
    ...
    description:
      "The MRR dropped 15% — investigate churn patterns and create retention experiments.",
...
function makePayload(
  overrides?: Partial<TaskSyncJobPayload>
): TaskSyncJobPayload {
  return {
    taskId: "task-1",
    ...overrides,
...
function createStubLinearClient(
  ...
  calls: Array<{ task: InternalTaskRow; teamId: string }>;
} {
...
function createThrowingLinearClient(
...
describe("task-sync processor", () => {
  ...
  function makeDeps(
    overrides?: Partial<TaskSyncProcessorDeps>
  ): TaskSyncProcessorDeps {
    return {
      taskRepo,
  ...
  describe("successful sync", () => {
    ...
    test("passes task title and description to Linear", async () => {
      ...
      expect(linearClient.calls[0]?.task.description).toContain(
        "MRR dropped 15%"
      );
    });
  });
  ...
  describe("idempotent handling of already-synced tasks", () => {
    test("skips sync when task is already synced", async () => {
      ...
      const skipLog = log.entries.find(
        (e) => e.msg === "task already synced — skipping"
      );
  ...
  describe("observability", () => {
    ...
    test("logs retryable failure with error and duration", async () => {
      ...
      const warnLog = log.entries.find(
        (e) => e.msg === "task sync failed (retryable)"
      );
