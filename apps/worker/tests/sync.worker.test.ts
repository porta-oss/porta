...
import { createStubProviderRouter, createThrowingProviderRouter } from '../src/providers';
...
function makeDeps(overrides?: Partial<SyncProcessorDeps> & { repo?: ReturnType<typeof createInMemoryRepo> }): SyncProcessorDeps & { repo: ReturnType<typeof createInMemoryRepo> } {
...
describe('sync processor', () => {
  describe('happy path', () => {
    ...
    test('records correct attempt number from job.attemptsMade', async () => {
      ...
      const runningOp = deps.repo.mutations.find((m) => m.op === 'markSyncJobRunning');
  ...
  describe('decryption failure', () => {
    test('marks job failed when encrypted config is corrupted', async () => {
      ...
      await expect(processor(makeJob(makePayload()))).rejects.toThrow('Decryption failed');
  ...
  describe('provider validation failure', () => {
    test('marks job failed (non-retryable) without throwing', async () => {
      const deps = makeDeps({
        validateProvider: createStubProviderRouter({ valid: false, error: 'Invalid API key' }),
      });
      const processor = createSyncProcessor(deps);
    ...
    test('marks job failed and throws when retryable', async () => {
      ...
      await expect(processor(makeJob(makePayload()))).rejects.toThrow('Server error');
  ...
  describe('durable failure state', () => {
    test('failed job records error text, timestamps, and duration', async () => {
      const deps = makeDeps({
        validateProvider: createStubProviderRouter({ valid: false, error: 'Auth failed' }),
      });
      const processor = createSyncProcessor(deps);
      ...
      const failOp = deps.repo.mutations.find((m) => m.op === 'markSyncJobFailed');
      expect(failOp).toBeDefined();
      const [syncJobId, connectorId, error, completedAt, durationMs] = failOp!.args as [string, string, string, Date, number];
    ...
    test('completed job records timestamps and duration', async () => {
      ...
      const completeOp = deps.repo.mutations.find((m) => m.op === 'markSyncJobCompleted');
      expect(completeOp).toBeDefined();
      const [syncJobId, connectorId, completedAt, durationMs] = completeOp!.args as [string, string, Date, number];
      expect(syncJobId).toBe('sjob-1');
      expect(connectorId).toBe('conn-1');
      expect(completedAt).toBeInstanceOf(Date);
      expect(typeof durationMs).toBe('number');
