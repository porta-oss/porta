...
export function readWorkerEnv(
  ...
  if (!VALID_RUNTIME_MODES.has(nodeEnv)) {
    throw new Error(
      `NODE_ENV must be one of: ${Array.from(VALID_RUNTIME_MODES).join(", ")}. Received: ${source.NODE_ENV}`
    );
  }
  ...
  return {
    ...
    databaseUrl: parseUrl(
      "DATABASE_URL",
      source.DATABASE_URL ?? DEFAULTS.DATABASE_URL,
      ["postgres:", "postgresql:"]
    ),
    redisUrl: parseUrl("REDIS_URL", source.REDIS_URL ?? DEFAULTS.REDIS_URL, [
      "redis:",
    ]),
    connectorEncryptionKey,
    workerConcurrency: parseInteger(
      "WORKER_CONCURRENCY",
      source.WORKER_CONCURRENCY ?? DEFAULTS.WORKER_CONCURRENCY,
      1,
      50
    ),
    jobTimeoutMs: parseInteger(
      "JOB_TIMEOUT_MS",
      source.JOB_TIMEOUT_MS ?? DEFAULTS.JOB_TIMEOUT_MS,
      1000,
      300_000
    ),
