 ...
 */
export function encryptConnectorConfig(plaintext: string, key: Buffer): EncryptedBlob {
 ...
 */
export function decryptConnectorConfig(blob: EncryptedBlob, key: Buffer): string {
  if (key.length !== KEY_BYTES) {
    throw new Error(`Encryption key must be ${KEY_BYTES} bytes.`);
  }
  ...
  if (iv.length !== IV_BYTES) {
    throw new Error(`Decryption failed: IV must be ${IV_BYTES} bytes, received ${iv.length}.`);
  }

  if (authTag.length !== AUTH_TAG_BYTES) {
    throw new Error(`Decryption failed: auth tag must be ${AUTH_TAG_BYTES} bytes, received ${authTag.length}.`);
  }
